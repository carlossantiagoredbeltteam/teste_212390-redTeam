#######################################################################################################################################################
# This module plots various shapes to polgons                                  .                                                                      #
#######################################################################################################################################################
"""
...
# Returns polygon for line
def line(line, keepToolOn = False ):
...
# Returns polygon for point
def point(point, keepToolOn = False ):
	poly = polygon.Polygon()
	x, y = point
	poly.addPoint( (x, y) )
	return poly

# Return polygon for arc with defined position, radius, start angle, end angle     resolution in lines per mm on circumference.
# This function works in degrees but takes parameters in radians
def arc(x, y, radius, startAngle, endAngle, resolution, fillDensity = False):
	poly = polygon.Polygon()
...
# Return polygon for filled ellipse with defined position, radius.
def ellipse(x, y, a, b, resolution, fillDensity = False):
	...
	for d in range( 1, numFills + 1 ):
		...
		for theta in frange( startAngle, endAngle + angleDiv, angleDiv ):
			...
			aLine = poly.addPoint(newX + x , newY + y)
...
def rectangle(x, y, width, height, fillDensity = False):
	...
	poly.addPoint( (cornerX, cornerY) )
	poly.addPoint( (cornerX, cornerY + height) )
	poly.addPoint( (cornerX + width, cornerY) )
	poly.addPoint( (cornerX + width, cornerY + height) )
	return poly

# Return polygon for filled photoplotter movement with circle aperture
def circleTrace(x1, y1, x2, y2, radius, resolution, fillDensity = False ):
	poly = polygon.Polygon()
	deltaY = y2 - y1
	deltaX = x2 - x1
	if x1 == x2 and y1 == y2:
		...
		for d in range( 1, numFills + 1 ):
			...
			poly.addPoint( (x2 - rsintheta, y2 + rcostheta) )
			poly.addPoint( (x1 - rsintheta, y1 + rcostheta) )
...
# Use potrace to convert raster to polygon(s)
def rasterToPolygon(fileName, originalWidth, originalHeight, svg = False):
	poly = polygon.Polygon()
	if svg:
