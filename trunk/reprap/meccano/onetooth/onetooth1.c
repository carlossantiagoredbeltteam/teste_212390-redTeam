...
#define BRS_SENSOR		128
#define FORWARD_ENDSTOP_SENSOR	64
#define	MAGNETIC_SENSOR		2
...
void main(void) {
	...
	// Set the I/O Direction magick
        TRISB = 0;
        TRISA = MAGNETIC_SENSOR+SPEED_SENSOR+FORWARD_ENDSTOP_SENSOR;
        count = 1;
	count2 = 1;
	xStepCount=0;
	...
	xState=XSTATE_INITIALISE;
		...
		// Fast, or slow?
		inport=PORTA;
		inport=inport&SPEED_SENSOR;
		if (inport==SPEED_SENSOR) {
			cdelay=4000;
		} else {
			cdelay=500;
		}
