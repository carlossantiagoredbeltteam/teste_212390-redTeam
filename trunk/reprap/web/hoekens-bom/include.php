					...
					switch ($part->type)
					{
						...
						case 'stud':
							...
							if (preg_match("/M(\d+)\D*(\d+)/", $part->name, $matches))
								$part->name = "M{$matches[1]} x {$matches[2]}mm $part->type";
							else
							{
								preg_match("/(\d+)/", $part->name, $matches); // match the length
						
								if ($part->type == 'rod')
									$part->name = "M8 x {$matches[1]}mm $part->type (fixme in BOM!)";
								else if ($part->type == 'stud')
									$part->name = "M5 x {$matches[1]}mm $part->type (fixme in BOM!)";
							}
						...
						case 'wire':
							if (preg_match("/(\d+)\D*(\d+)AWG/", $part->name, $matches))
								...
								$part->name = "{$matches[1]}mm wire (specify AWG in BOM!)";
