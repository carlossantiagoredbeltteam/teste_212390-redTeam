...
class cartesian
{
	...
	if(henv.length() <= 0)  
	    ...
	    {
		state_file = henv + "/.new-cartesian";
		File f = new File(state_file);
		if(!(f.exists() && f.isDirectory()))
		    {
			File st = new File(henv, "/.new-cartesian");
			st.mkdir();
    ...
    // Convert 16-bit numbers to/from hex
    ...
    {
	n = n & 0xffff;
	return zPad(Integer.toString(n, 16).toLowerCase(), 4);
    }
    ...
    public void set_speed(int speedcount)
    {
	String s;
