...
class cart_test
{
	...
	int speedcount, acccount, inccount, stcount, loopcount;
	byte b;
	int stp, lp, rz;
	String f_name;
	String k;
	...
	rr_3point rr = new rr_3point(c.envelope());
	rr_3point rrr;
	...
	do
		...
		if(k.equals("p"))
		    ...
		    {
			System.out.print("X, Y, Z for start: ");
			r.x = in.getDouble();
			r.y = in.getDouble();
			r.z = in.getDouble();
			System.out.print("X, Y, Z for end: ");
			rr.x = in.getDouble();
			rr.y = in.getDouble();
			rr.z = in.getDouble();
			if(!c.go(r))
			...
			System.out.print("Number of times to loop: ");
			loopcount = in.getInt();
			...
			System.out.println("r - loop to and fro between 2 points");
